...
```json
{
  ...
  "logType": "process",              ❌ 当前："agent-audit"
  "opType": "create",                ✓ 已实现
  "localTime": "2025-12-23 10:51:12", ✓ 已实现（ISO8601格式）
  "unixTime": 1766458272,            ✓ 已实现
  "logfuzId": "...",                 ✓ 已实现（MD5前16字符）
  ...
  "initialProcess": "0",             ❌ 缺失
  "destUserId": "0",                 ❌ 缺失
  "currentDirectory": "/",           ✓ 已实现（processCwd）
  "teletype": "pts4",                ❌ 缺失
}
```

**改进建议：**
1. `logType` 改为 `"process"`
2. 添加 `processStartTime`（进程启动时间，从agent_tree的fork_time）
3. 添加父进程完整信息（从/proc/{ppid}读取）
4. 添加终端信息（relateAddress从SSH会话，loginId从utmp，teletype从tty）
...
```json
{
  ...
  "createTime": "",                  ❌ 缺失
  "modifyTime": "...",               ❌ 缺失
  "targetFilename": "/11111111.sh",  ❌ 缺失（完整路径）
}
```
...
2. **networkDst 格式修复**
   - 当前：`"family=1 (raw)"`
   - 规范：`"192.168.27.242:22"`（destAddress + destPort）
   - 需要解析BPF sockaddr结构体

3. **dnsQuery opType 修正**
   - 当前：`"resolve"`
   - 规范：`"connect"`

4. **字段名修正**
   - `dnsQuery` → `requestDomain`
   - `processCwd` → `currentDirectory`
